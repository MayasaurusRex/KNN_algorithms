...
#
##################### PLEASE SEE ###################
# This notebook is used for applying KNN using Manhattan distance for ~24 million training points and 2,135 test points
...
# read in the test data to the knn manhattan dataframe
knn_manhattan = pd.read_csv('test_data.csv', header = None, names = ['Cust_Id','Movie_Id'])

# change the parameters from strings to integer values
knn_manhattan['Cust_Id'] = knn_manhattan['Cust_Id'].astype(int)
knn_manhattan['Movie_Id'] = knn_manhattan['Movie_Id'].astype(int)
...
for row in knn_manhattan.itertuples():
    # calculate the manhattan distance between the test point and all the training points,
    # store this value as a new column (this helps to parallelize it and make more efficient)
    df_predictions['distances'] = (abs(row.Cust_Id-df_predictions['Cust_Id'])+ abs(row.Movie_Id-df_predictions['Movie_Id']))
